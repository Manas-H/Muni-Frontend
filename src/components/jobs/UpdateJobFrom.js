  ...
  useEffect(() => {
    const fetchJob = async () => {
      ...
      try {
        ...
        setSelectedOptions(response.data.jobPreference || {});
        setFormData((prevData) => ({
          ...prevData,
          ...response.data,
          jobPreference: response.data.jobPreference || {},
        }));
      } catch (err) {
      ...
      <div className="bg-white border shadow-md rounded-lg m-2 p-2">
        <form>
          ...
          <div className="flex items-end justify-end mx-7 my-5">
            <span
              className="cursor-pointer box_shadow border border-blue-300 rounded-md py-2 px-7 text-[#5f6d79] hover:text-white hover:bg-[#1ac790]"
              onClick={handleSubmit}
            >
              Update <i className="pl-2 fa fa-arrow-right"></i>
            </span>
          </div>
        </form>
      </div>
